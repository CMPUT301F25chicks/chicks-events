...
@LargeTest
public class TEST02_US010201_EntrantProvideInfoUITest {
    ...
    /**
     * Test: Entrant can save personal information.
     * Verifies that clicking save button actually saves the data.
     */
    @Test
    public void entrant_canSavePersonalInfo() {
        navigateToProfileForm();

        // Enter information
        onView(withId(R.id.edit_name)).perform(typeText("Test User"), closeSoftKeyboard());
        onView(withId(R.id.edit_email)).perform(typeText("test@example.com"), closeSoftKeyboard());
        onView(withId(R.id.edit_phone)).perform(typeText("5551234567"), closeSoftKeyboard());

        // Verify save button exists and is clickable
        waitForView(withId(R.id.btn_save_info), 5);
        onView(withId(R.id.btn_save_info))
        ...
        try {
            Thread.sleep(3000); // Wait for Firebase save
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        }

        // VERIFY FUNCTIONALITY: Data should be saved
        // After saving, verify the fields still contain the entered data
        // (This verifies the save operation completed)
        ...
        // Note: Full verification would require navigating away and back to verify
        // data persists, but this verifies the save button works and data is retained
    }
