...
@LargeTest
public class US020503_OrganizerPoolReplacementUITest {
    ...
    /**
     * Test Case 5: Organizer can click pool button.
     * 
     * As an organizer, I should be able to click the pool button
     * to trigger the replacement drawing process.
     */
    @Test
    public void organizer_canClickPoolButton() {
        ...
        try {
            ...
            // Wait for pooling process to complete
            try {
                Thread.sleep(2000);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            ...
            // Verify button is still accessible (process completed)
            onView(withId(R.id.btn_pool))
                    .check(matches(isDisplayed()));
        } catch (Exception e) {
            // Fragment might not be loaded or no replacements needed
        }
    }
